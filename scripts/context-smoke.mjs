    ...
    event('response.completed', {
      response: {
        ...
        usage: {
          ...
          output_tokens: 6,
          total_tokens: reportedInputTokens + 6,
...
try {
  ...
  await rpc('turn/start', {
    ...
    input: [{ type: 'text', text: 'Remember our task: build a calculator.', text_elements: [] }],
  });
  await waitTurn(before);
  before = events.length;
  assert.deepEqual(await rpc('thread/compact/start', { threadId: started.thread.id }), {});
  await waitTurn(before);
  assert.ok(
    events
      .slice(before)
      .some((e) => e.method === 'item/completed' && e.params.item.type === 'contextCompaction'),
  );
  before = events.length;
  await rpc('turn/start', {
    threadId: started.thread.id,
    input: [{ type: 'text', text: 'Continue the task.', text_elements: [] }],
  });
  await waitTurn(before);
  const usage = events.findLast((e) => e.method === 'thread/tokenUsage/updated');
  assert.ok(usage.params.tokenUsage.modelContextWindow > 0);
  console.log(
    'PASS native manual compaction and continued conversation; context window:',
    usage.params.tokenUsage.modelContextWindow,
  );
  assert.equal(usage.params.tokenUsage.modelContextWindow, 190000);
  reportedInputTokens = 170000;
  before = events.length;
  await rpc('turn/start', {
    threadId: started.thread.id,
    input: [{ type: 'text', text: 'Grow context for automatic compaction.', text_elements: [] }],
  });
  await waitTurn(before);
  ...
  await rpc('turn/start', {
    ...
    input: [
      {
        type: 'text',
        text: 'Continue after reaching the configured threshold.',
        text_elements: [],
      },
    ],
  });
  ...
  assert.ok(
    events
      .slice(before)
      .some((e) => e.method === 'item/completed' && e.params.item.type === 'contextCompaction'),
    'Automatic compaction must occur after the configured threshold',
  );
  await assert.rejects(rpc('thread/compact/start', { threadId: 'invalid-thread' }));
  console.log(
    'PASS native automatic compaction at configured threshold and invalid-thread rejection',
  );
  await rpc('thread/archive', { threadId: started.thread.id });
  console.log('PASS: context budgets and native compaction. Fixture service only.');
} catch (error) {
