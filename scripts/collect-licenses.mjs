...
async function collect(name, version, license, directory, extra) {
  const files = (await readdir(directory)).filter((name) =>
    /^(licen[cs]e|copying|notice|unlicense|copyright)/i.test(name),
  );
  ...
  for (const file of files) {
    try {
      ...
      /* Directory names are not license files. */
    }
  }
...
for (const packages of Object.values(npm)) {
  for (const pkg of packages)
    for (let i = 0; i < pkg.paths.length; i++)
      await collect(pkg.name, pkg.versions[i] ?? pkg.versions[0], pkg.license, pkg.paths[i]);
}
for (const pkg of rust.packages.filter((p) => p.id !== rust.resolve.root))
  await collect(
    pkg.name,
    pkg.version,
    pkg.license ?? 'LicenseRef-Custom',
...
await writeFile(
  resolve(root, 'docs/dependency-inventory.json'),
  JSON.stringify(inventory, null, 2) + '\n',
  'utf8',
);
await writeFile(
  resolve(root, 'work/missing-license-files.json'),
  ...
  JSON.stringify(
    {
      ...
      licenseExpressions: [...new Set(inventory.map((p) => p.license))],
      packagesWithoutStandaloneLicenseFile: missing.map((p) => `${p.name}@${p.version}`),
    },
    null,
    2,
  ),
