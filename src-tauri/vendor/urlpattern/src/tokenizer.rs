...
use crate::Error;
use icu_properties::{props::{IdContinue, IdStart}, CodePointSetDataBorrowed};
...
static ID_START: CodePointSetDataBorrowed<'_> = CodePointSetDataBorrowed::new::<IdStart>();
static ID_CONTINUE: CodePointSetDataBorrowed<'_> = CodePointSetDataBorrowed::new::<IdContinue>();
...
mod unicode_regression_tests {
  use super::is_valid_name_codepoint;
  #[test]
  fn ecmascript_identifier_boundaries() {
    for c in ['a', '中', '$', '_'] { assert!(is_valid_name_codepoint(c, true)); }
    for c in ['1', '\u{0301}', '\u{200c}', '\u{200d}', '\u{30fb}'] {
